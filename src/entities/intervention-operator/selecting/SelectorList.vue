            ...
            <div class="row border-bottom border-bottom-1 py-2" :class="{ 'is-selected': isSelected(item) }">
                ...
                <div class="col text-truncate">
                    ...
                    <div class="d-md-none text-muted">
                        <AddressButton v-if="items[i].$address" :modelValue="items[i].$address!" class="btn btn-default p-1" />
                        {{ items[i].$address?.city || items[i].$address?.countryCode }}
                    </div>
                </div>
            </div>
        </template>
...
import { AddressButton } from "../operator-addresses"
