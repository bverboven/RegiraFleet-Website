    ...
    <Autocomplete v-model="item" :search="search" :max-results="maxResults" :id-selector="idSelector" :display-item-formatter="displayItemFormatter" :placeholder="placeholder" ref="autoEl">
        <template #default="{ item, q }">
            <div class="row">
                ...
                <div v-if="item?.$address" class="col d-none d-md-block text-end text-truncate">
                    <Icon name="map" class="me-1" :title="item?.$address.city || item?.$address?.countryCode" /> {{ item?.$address?.city || item?.$address?.countryCode }}
                </div>
            </div>
        </template>
    </Autocomplete>
